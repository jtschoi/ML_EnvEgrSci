...
def open_dataset(file_path):
    ...
    IMPORTANT: For this to work the `file_path` must be provided without
    extension.
    """
    if "gs://" in file_path:
...
def prepare_predictand(data_sets, data_path, time_reindex=True):
    ...
    Y = (
        Y.rename({"lon": "longitude", "lat": "latitude"})
...
def get_rmse(truth, pred):
    ...
    return np.sqrt(
        ((truth - pred) ** 2).weighted(weights).mean(["latitude", "longitude"])
    ).data.mean()
